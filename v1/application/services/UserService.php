 ...
 *
 * This is the User Service layer Business Logic
 *
 * @author johannes
 */
class UserService {
    ...
    /**
     * completeRegistration - complete user registration service
     * @param array $data - input data
     * @return array - json encoded array
     */
    ...
    /**
     * resetPasswordCode - generates and send user password reset code to user email
     * @param array $data - input data
     * @return array - json encoded array
     */
    public function resetPasswordCode($data){
        try {
            $this->cnx->beginTransaction();
            $user_model = new UsersModel($this->cnx);
            $user_profile_model = new UserProfileModel($this->cnx);
    ...
    /**
     * confirmResetCode - confirms code for password reset
     * @param $data
     * @return string
     */
    public function confirmResetCode($data){
        try {
            $this->cnx->beginTransaction();
            $user_code_model = new UserCodeModel($this->cnx);
            $user_code_model->confirmCode($data["user_id"], $data["code"]);
            Utils::userLogEntry($data["user_id"], "Password reset code sent", $this->cnx);
            $this->cnx->commit();
            return Utils::response([
                "message" => "Successful",
                "status" => "200"
    ...
    /**
     * resetPassword - Service to reset password
     * @param $data
     * @return string
     */
    public function resetPassword($data){
        try {
            ...
            $user_profile_model = new UserProfileModel($this->cnx);
            $secure_hash = new SecurePasswordHash();
            $user = $user_model->getByUserId($data["user_id"]);
            if(!$user){
                Utils::systemLogEntry("Invalid user email: {$data["email"]}", $this->cnx);
                $this->cnx->commit();
                return Utils::response([
            ...
            $date = Utils::formateDateTime(Utils::getDateTime());
            $location = Utils::getUserLocation();
            $body = "
                <p>Hi $user_profile->first_name,</p>
                <p>You've successfully changed your " . APP_NAME . " login password.</p>
                <p>When and where this happened:</p>
                <p>Date: $date<br/>Approximate Location: $location</p>
                <br/>
                <p>Kind Regards,</p>
                <p>The " . APP_NAME . " Support Team</p>
                <p>Contact us anytime at " . SUPPORT_EMAIL . "</p>
            ...
            Utils::userLogEntry($user->user_id, "Password reset successful", $this->cnx);
            $this->cnx->commit();
            return Utils::response([
                "message" => "Successful",
                "status" => "200"
